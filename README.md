...
```
solver service starting: workers=8 max_concurrent_jobs=1 stage_targets=<none> stage_budget_s=<engine-default> mode_a_budget_s=<engine-default> ... credential_configured=True wire_contract=loaded
...
Optional: `SOLVER_WORKERS` (default `8`, pinned for reproducibility — never auto), `SOLVER_MAX_CONCURRENT_JOBS` (default `1`; further dispatches get a 503), `SOLVER_LOG_LEVEL` (default `INFO`), `SOLVER_STAGE_TARGETS` (default empty) and the two ladder budgets `SOLVER_STAGE_BUDGET_S` / `SOLVER_MODE_A_BUDGET_S` (default: unset).

`SOLVER_STAGE_TARGETS=tier=value[,tier=value]` — e.g. `3=95,6=900` — stops those ladder stages as soon as the objective reaches the value instead of burning the whole budget; the stage records `stoppedBy: "target"`. Tiers outside 2–10 and malformed entries are dropped with a complaint on stderr rather than refusing to start. **Left unset, behaviour is exactly what it was before S-303**, and production forwards the key empty on purpose: a target is an objective VALUE, so it belongs to a catalog and a planning season rather than to a deployment constant.

`SOLVER_STAGE_BUDGET_S` and `SOLVER_MODE_A_BUDGET_S` are the ladder's time allowances in seconds — one per polishing stage, one for the tier-1 completeness solve that precedes them. **Unset means the engine's own default**, and this is the one place that phrasing is literal: the runner does not pass the field at all, so the literals in `SolveConfig` (`services/solver/src/cpsat_engine/solve.py`) stay the single source of truth and no second copy exists to drift. A malformed or non-positive value degrades back to unset with a complaint, like every other knob here. Setting `SOLVER_STAGE_BUDGET_S=5` is the cheapest way to watch a full ten-stage ladder locally.

> **Production's values live in the Worker, not here.** `src/solver-container-env.ts` pins `CONTAINER_STAGE_BUDGET_S`, `CONTAINER_MODE_A_BUDGET_S` and `CONTAINER_STAGE_TARGETS` and forwards all three into the container, so the deployed configuration is visible in the repo and the container's startup line proves which values a run actually used. A change to one of those constants is a Worker-only deploy and takes effect **at the container's next cold start** — a warm container keeps solving under the values it booted with. And the standing rule still binds: an M-series number must never become a container's budget (see the campaign note below).
...
**No path filters anywhere in this workflow**, and that is deliberate rather than an oversight. The four test jobs carry no `needs:` between them, so they run in parallel and the critical path is `e2e` at ~444 s (measured 2026-08-28 on PR #120 — the solver boot S-306 added costs ~18 s over the previous ~426 s, because `uv sync` is cached and the generation fixture solves in ~1 s); the `solver` job is 44 s, so filtering it saves billed minutes and zero wall clock — while requiring a rewrite of `deploy`'s `if:` gate that would switch off the implicit success check on all four needs at once, on the only job that touches production. The accepted cost is that the image build runs on every merge, doc-only merges included. If that becomes a problem the lever is GitHub Actions Docker layer caching (`cache-from`/`cache-to: type=gha`), **never** path filters.
