...
- ~~Does Astro 7 bundle a new `zod` major?~~ **RESOLVED (Phase 1).** Astro 7.0.6 still bundles `zod@4.x`; the project's direct `zod@4.4.3` stays aligned — no coordinated bump needed. Confirmed via the installed tree (`zod@4.4.3` direct; the only other `zod` in the graph is a transitive `3.25.76`, unrelated to the Astro alignment).
- **Decision: can `ssrPrebundleDeps` be removed entirely? → NO. RESOLVED (Phases 2–3): plugin is REQUIRED and had to be RE-TUNED.** The empirical cold-start protocol (`rm -rf node_modules/.vite`, cold-start the Astro 7 dev daemon, authenticated first-request SSR of `/plans/:id`, ≥3 cold starts) gave a decisive result:
  - **The primary risk materialized.** Under Vite 8's **Rolldown** dep-optimizer (replacing esbuild), Astro's virtual runtime modules — `astro/actions/runtime/entrypoints/server.js` (`astro:actions`) and `astro/virtual-modules/transitions-*.js` (`astro:transitions/client`) — are resolved by Astro's Vite plugin at transform time, so Rolldown's static pre-scan can't see them and discovers them **late on the first render**. Each late discovery re-optimizes mid-render and reloads `react-dom/server` on a fresh `?v=` hash → two React instances → *"Invalid hook call" / "Cannot read properties of null (reading 'useContext')"* (surfaced via `useLucideContext`, the first context consumer — a victim, not the trigger). `/auth/signin` (bare auth layout, no `astro:actions`/`navigate()`) stayed clean; `/plans/:id` (board island using both) crashed. Vite 7's esbuild optimizer had pre-bundled these; Rolldown does not.
  - **Removal experiment (Phase 3), direct evidence:** with the plugin removed from `vite.plugins`, `/plans/:id` failed on **all 3 cold starts** — `reloads=3, crashes=23`, late-optimizing exactly `astro/env/runtime`, `astro/actions/runtime/entrypoints/server.js`, and the four `transitions-*` modules. With the re-tuned plugin restored, all 3 cold starts were **clean** (`reloads=0, crashes=0`, first-request SSR `200`).
  - **Applied decision:** keep the plugin, and **extend `optimizeDeps.include`** with the astro runtime entries above (per the memory-note recipe — extend the include list rather than revert blindly). This is the app's complete set of island-imported virtual modules per the documented framework surface (`astro:env/server`, `astro:actions`, `astro:transitions/client`), so the fix is bounded. `astro.config.mjs` reflects this (plugin present + re-tuned); the plugin comment documents the Vite 8 root cause. Landed in Phase 2 commit `c274824`.
- ~~Confirm `navigate` is still exported from `astro:transitions/client` in v7~~ **RESOLVED (Phase 2): yes.** The catalog URL-synced filter (`navigate(pathname + search)`) soft-navigated `/plans/:id/teachers` to `?q=Physics` and re-ran the loader in-browser with a clean console — `navigate` is exported and functional in v7.
