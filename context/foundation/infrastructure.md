...
- **Secrets**: `wrangler secret put SUPABASE_URL` and `wrangler secret put SUPABASE_KEY` (the **Publishable** key) for production; `.dev.vars` (gitignored, present in `.gitignore` from scaffold) for local. Per-environment via `--env`. **The Secret (service-role) key is deliberately NOT pushed to the Worker** — the app authorizes every request through the author's cookie session with RLS live, so a key that bypasses RLS would only widen the blast radius of a Worker compromise. That is the decision of record in `context/deployment/deploy-plan.md` (Phase 5.3); this research pre-dated it and originally recommended the opposite. The CP-SAT solver, which needs a non-cookie credential, gets a narrow Postgres role instead of a privileged key — see `docs/runbooks/solver-credential.md`. GitHub Actions reads `CLOUDFLARE_API_TOKEN` from repo secrets — token scoped to `Workers Scripts: Edit` for this project only, no DNS, no account-level access.
...
4. **Set production secrets**: `pnpm exec wrangler secret put SUPABASE_URL`, then `pnpm exec wrangler secret put SUPABASE_KEY` (the Publishable key — the variable name the app actually reads). Those two are the whole set: the Secret/service-role key is not pushed (see Operational Story → Secrets). Copy the same names to `.dev.vars` (gitignored) for local dev, or just run `pnpm env:local`.
