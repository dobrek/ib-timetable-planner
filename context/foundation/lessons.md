...
- **Rule**: Anything the Worker must forward to a container has to be written into `.dev.vars` BEFORE `pnpm build` — never merely exported. And when a service deliberately boots without configuration, put the fail-fast guard in the launcher (the launcher script a mise task invokes — `scripts/solver/*.sh` since 2026-08-18 — or a CI step), not in the service, so the developer-machine trap closes without breaking the bare-container promise.
- **Applies to**: plan, plan-review, implement, impl-review
