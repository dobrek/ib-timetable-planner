...
- **Rule**: **Form-based catalog/entity CRUD → Astro Actions** (`src/actions/`, `defineAction` + Zod `input`), called from React islands via `import { actions } from 'astro:actions'`; client forms use **react-hook-form + `@hookform/resolvers/zod` + shadcn `<Form>`/`<Table>`/`<Dialog>`**, with the **Zod schema shared from `src/lib/schemas/`** between the action `input` (authoritative server gate) and the form resolver. **Realtime / optimistic mutation hot paths** (drag-drop placements, grouping compute) **→ stay as `src/pages/api/*.ts`** routes (e.g. `placements.ts`, `grouping.ts`) — do NOT migrate them to Actions. Validation lib is `import { z } from 'astro/zod'` (version-aligned with Astro). Adopting Actions does not require retrofitting the existing API routes.
