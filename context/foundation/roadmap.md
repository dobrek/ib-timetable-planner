...
| S-306 | drift-decided-delivery          | review a finished board and decide — merge it into the source or keep it as a plan; drift gates the merge  | S-301                      | FR-306, FR-307, FR-313, US-301, US-303                                | proposed |
...
| D      | Delivery, policy & notification     | `S-306` → `S-310`, with `S-307` parallel | Author-decided delivery and the launch-time policy picker both hang directly off `S-301`; email tails delivery, and since 2026-08-28 also carries the walk-away trigger `S-306` no longer owes. |
...
### S-306: Author-decided delivery, drift-gated

- **Outcome:** On job completion the author is notified in-app that a result is waiting, opens it as a **board they can read**, and decides: **merge** it into the source via the atomic RPC, or **keep** it as a separate plan. The drift guard is promoted to production and runs at that moment to gate and inform rather than to decide — an **unchanged** source makes merging a single confirmation; a **changed** source is named as drifted, and merge is offered only where the board still passes the oracle against the source as it now stands. Review happens on the existing comparison page, reached from the completed job. Every write runs inside the author's own authenticated request.
- **Change ID:** drift-decided-delivery
- **PRD refs:** FR-306, FR-307, FR-313, US-301, US-303
- **Prerequisites:** S-301
- **Parallel with:** S-303, S-304, S-305, S-307
- **Blockers:** —
- **Unknowns:** **(1) When the proposal is materialised** — dispatch (today) or completion. PRD Open Question 4; the clone has no reader before completion but is the only durable copy of the display catalog the snapshot omits. **(2) What the notification event is** — a durable row transition (`notified_at` exists, unused) or a render. A render silently de-scopes S-310, which is specified to extend the event created here.
- **Risk:** The delivery decision is where a stale board could reach the database if the drift gate is wrong — the oracle + apply-time re-verify remain the trust boundary regardless of what the container returns. Reuses the existing comparison page as the review surface (FR-306 — no new side-by-side UI); that saving is real, but the residual is bigger than "deliberately small" implied: the review path is now **100% of runs** and today has no route in from a completed job, no drift signal on a source-vs-proposal pair (the page's banner fingerprints the catalog, and a board-only edit does not move it), and no act to conclude with. That, not the delivery mechanics, is the slice's centre of gravity.
- **Status:** proposed
...
- **Blockers:** —
- **Unknowns:** **This slice now solely owns dominance** *(moved here 2026-08-28 from FR-306/S-306 by S-306's frame — it is where competing policies make the comparison bite; S-306's single-result review does not need it)*. It is net-new at every layer and larger than the Outcome sentence implies: no dominance code exists in the app, solver, contract or bench; `compareObjectives` is **lexicographic**, which is a different relation and not a substitute; the wire contract explicitly excludes the finished board's objective tuple (`contracts/generation-wire.schema.json:180`), so both tuples must be computed app-side via `scoreCandidate`, whose `remaining` input must be re-derived from `analyzePlan`'s completeness; and `scoreCandidate` lives in `objective.ts` alongside greedy, so **S-309's deletion forces a call on whether that module is engine code or product code**. If dominance is ever surfaced on the comparison page, note that page's documented refusal to judge — restated in five files and physically enforced by every metric formatting to a finished string — so it needs a written override, not an incremental addition.
...
| S-306      | drift-decided-delivery          | [#103](https://github.com/dobrek/ib-timetable-planner/issues/103) | Author-decided delivery: review the board, merge to source or keep as a plan; drift gates the merge | no                    | **Unblocked — S-301 done. Re-grounded 2026-08-28**: research + frame are written (`context/changes/drift-decided-delivery/`), auto-apply is retired and dominance moved to S-307. Run `/10x-plan drift-decided-delivery`; settle PRD Open Question 4 (materialisation timing) and the notification-event shape in the plan. Issue #103's title/body still describe auto-apply and need updating |
