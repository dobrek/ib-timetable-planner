...
4. **Tests are the refactor safety net, never a refactor blocker.** This
   codebase is early and substantially LLM-generated; a structural refactor
   is anticipated and not yet done. Every test the rollout adds must assert
   **observable behavior at a stable boundary** — the validator's verdict,
   the user-visible drop feedback, auth/RLS enforcement, persistence
   durability — the contracts that survive a refactor. Tests must **never**
   mirror internal structure that is about to move. If a test would have to
   change purely because internals were reshaped and no behavior changed, it
   is the wrong test. The sequencing this implies: build the behavioral net
   (Phases 1–2) first, then refactor under it. The refactor itself is out of
   scope for this rollout; the rollout exists to de-risk it. This is the
   direct answer to interview Q2 and the gate Phase 4 enforces.
...
|---|--------------------------|--------|------------|--------------------------------|
| 1 | The validator marks a placement **valid that actually collides** for a student (the shipped student-collision class) — the author trusts and ships a broken timetable. | High | High | PRD §Guardrails L56 (no wrong-positive); interview Q1; hot-spot dir `src/lib/placements`, `src/pages/api` |
| 2 | The **drag → API → grid feedback loop renders the wrong verdict** — an optimistic UI keeps showing "valid" after the server returns invalid, so the user never sees the collision. | High | High | interview Q3 + Q4; hot-spot dir `src/components/planner` (11 commits/30d), `src/pages/api` (8 commits/30d) |
| 3 | A **server-mutation boundary diverges from its tested core** — across *both* Astro Actions (`src/actions/`, catalog CRUD) and API routes (`src/pages/api/`, validator/grouping hot path): broken auth enforcement, wrong `DomainError`→`ActionError` translation, failed persistence, off-contract response, or no server-side re-validation of untrusted input. | High | Medium | interview correction (Actions are a Supabase boundary too); test base: 0 integration tests in `src/actions` or `src/pages/api`; lessons.md two-mutation-styles rule |
| 4 | **Placed work is lost** — a placement does not survive reload/crash, or a write is silently dropped, violating the work-durability promise. | High | Medium | PRD NFR Work durability L129; hot-spot dir `src/pages/api` |
| 5 | **Unauthenticated or cross-author access to student PII or the catalog** — an RLS ownership gap, or an Astro Action handler missing `requireSession()` (the `/_actions/*` path is exempt from the middleware redirect by convention, so the gate is per-handler). *(abuse: authorization / IDOR)* | High | Medium | PRD NFR Data privacy L130, Access Control L167; Actions self-enforce auth (interview correction + `src/actions` convention); auth untested; hot-spot dir `src/components/auth` (12/30d), `src/pages/auth` (6/30d) |
...
|------|-----------------------------|----------------|--------------------------------------|-----------------------|-----------------------|
| #1 | A hand-built fixture where two students share a course across one slot is **rejected**; a clean fixture is accepted. | "`validate.test.ts` already covers this" — does it, or does its oracle mirror the implementation? | Where the collision rule lives; whether the existing test's expected values come from requirements or from the code under test. | unit (core) | **Oracle problem** — expected values lifted from the validator's own code; coupling to internal structure rather than the verdict contract. |
...
| #3 | A request hitting **each boundary type** (a real Astro Action *and* the placements API route) enforces auth, translates a domain error to the correct client error, persists, and rejects malformed/untrusted input. | "the domain function is tested, so the boundary is fine" — the *handler wiring* is what is untested. | The real handler path for each mechanism; the persistence call; the response/error contract shape; the `requireSession` enforcement point. | integration (real Action via Astro's action context / real route, local Supabase). | Mocking the domain function *inside* the boundary test (tests nothing); re-testing `src/lib/courses` logic that is already covered. |
