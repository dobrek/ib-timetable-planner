...
// ---------------------------------------------------------------------------
// Per-plan emission — fresh UUIDs every pass
// ---------------------------------------------------------------------------
function emitOverlaps(overlapRows, courseIds, catalog, planId) {
  const rows = [];
  const seen = new Set();
  for (const cols of overlapRows) {
    const baseName = cols[0],
    ...
    seen.add(pairKey);
    rows.push([q(randomUUID()), q(planId), q(courseIds.get(baseKey)), q(courseIds.get(depKey))]);
  }
  return rows;
}

function emitMerges(mergeRows, courseIds, catalog, planId) {
  const rows = [];
  const seen = new Set();
  for (const cols of mergeRows) {
    ...
    if (seen.has(pairKey)) continue;
    seen.add(pairKey);
    rows.push([q(randomUUID()), q(planId), q(courseIds.get(parentKey)), q(courseIds.get(childKey))]);
  }
  return rows;
}

function emitChoices(studentRows, studentIds, courseIds, choiceResolution, catalog, planId) {
  const rows = [];
  const seen = new Set();
  for (const cols of studentRows) {
    ...
    if (seen.has(pairKey)) continue;
    seen.add(pairKey);
    rows.push([q(randomUUID()), q(planId), q(studentId), q(courseId)]);
  }
  return rows;
}

// Runs the full insert pipeline for one plan: fresh plan UUID, fresh entity
// UUIDs, every row keyed (directly or denormalized) to the plan.
function emitPlan(planName, dp1Data, dp2Data, fixtures) {
  ...
  sql.push(inserts("plans", ["id", "name", "slot_grid_preset"], [[q(planId), q(planName), q("5x10")]]));
  ...
  sql.push(
    inserts(
      "teachers",
      ["id", "plan_id", "code"],
      teacherRows.map(([code, id]) => [q(id), q(planId), q(code)]),
    ),
  );

  // courses — Year 1 then Year 2
  function emitCourses(catalog, courseIds, cohort) {
    const rows = [];
    for (const [k, c] of catalog) {
      ...
      rows.push([q(id), q(planId), q(cohort), teacherId, q(c.name), q(c.level), c.group_index, c.hours_per_week]);
    }
    return rows;
  }
  const courseRows = [
    ...emitCourses(dp1Data.catalog, courseId1, COHORT_DP1),
    ...emitCourses(dp2Data.catalog, courseId2, COHORT_DP2),
  ];
  sql.push(
    inserts(
      "courses",
  ...
  // course_overlaps
  const overlapRows1 = emitOverlaps(fixtures.dp1Overlaps, courseId1, dp1Data.catalog, planId);
  const overlapRows2 = emitOverlaps(fixtures.dp2Overlaps, courseId2, dp2Data.catalog, planId);
  if (overlapRows1.length + overlapRows2.length > 0) {
    sql.push(
      inserts(
        "course_overlaps",
        ["id", "plan_id", "base_course_id", "dependent_course_id"],
        [...overlapRows1, ...overlapRows2],
      ),
    );
  }

  // course_merges
  const mergeRows1 = emitMerges(fixtures.dp1Merges, courseId1, dp1Data.catalog, planId);
  const mergeRows2 = emitMerges(fixtures.dp2Merges, courseId2, dp2Data.catalog, planId);
  if (mergeRows1.length + mergeRows2.length > 0) {
    sql.push(
      inserts(
        "course_merges",
        ["id", "plan_id", "parent_course_id", "child_course_id"],
        [...mergeRows1, ...mergeRows2],
      ),
    );
  }

  // students
  const studentRows1 = [...studentId1.entries()].map(([name, id]) => [q(id), q(planId), q(COHORT_DP1), q(name)]);
  const studentRows2 = [...studentId2.entries()].map(([name, id]) => [q(id), q(planId), q(COHORT_DP2), q(name)]);
  sql.push(inserts("students", ["id", "plan_id", "cohort", "full_name"], [...studentRows1, ...studentRows2]));

  // student_choices
  const choices1 = emitChoices(
    dp1Data.studentRows,
    studentId1,
    courseId1,
  ...
  sql.push(inserts("student_choices", ["id", "plan_id", "student_id", "course_id"], [...choices1, ...choices2]));
...
for (const planName of PLAN_NAMES) {
  const { sql, stats } = emitPlan(planName, dp1Data, dp2Data, fixtures);
  out.push(...sql);
