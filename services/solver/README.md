...
| `SOLVER_STAGE_TARGETS`       | _empty_                           | `tier=value[,tier=value]`, e.g. `3=95,6=900`. Stops those ladder stages once the objective reaches the value instead of burning the budget; tiers outside 2–10 and malformed entries are dropped with a stderr complaint. Empty = pre-S-303 behaviour exactly. Production forwards the key **empty** (`CONTAINER_STAGE_TARGETS`): a target is an objective value, so it belongs to a catalog and a season rather than to a deployment constant. |
| `SOLVER_STAGE_BUDGET_S`      | _unset_ → `SolveConfig`           | Seconds each of the nine ladder stages may run. Unset means the engine's own literal — the runner does not pass the field at all — so this file names no number; `cpsat_engine/solve.py`'s `SolveConfig` is the single source of truth. Malformed or non-positive degrades to unset with a stderr complaint. Production's value is `CONTAINER_STAGE_BUDGET_S` in `src/solver-container-env.ts`. |
| `SOLVER_MODE_A_BUDGET_S`     | _unset_ → `SolveConfig`           | The same, for the tier-1 completeness solve that runs before the ladder. Production's value is `CONTAINER_MODE_A_BUDGET_S`. |
