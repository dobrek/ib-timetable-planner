...
/**
 * The headline finding of the v0 report, and the dimension the `Objective` tuple has no notion of:
 * the expert's boards carry hundreds of same-course adjacent pairs (double periods) and **zero**
 * same-day splits — "never a gap between two hours of the same subject" is an invariant, not a
 * preference. Subject identity is NOT needed: a student takes at most one course per subject, so
 * this is a same-`courseId` phenomenon at the row grain.
 *
