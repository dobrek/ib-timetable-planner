-- clone_plan, extended to carry slot_bundles overrides.
--
-- Full body copied verbatim from 20260611180100_clone_plan_fn.sql (create or
-- replace), with one added statement that copies the source plan's unbundled
-- overrides by coordinate. No temp map needed — slot_bundles has no UUID members
-- to remap; the cell coordinate (cohort/day/period) is copied as-is, exactly like
-- placements copy their coordinate. Keeps SECURITY INVOKER and the same signature.
