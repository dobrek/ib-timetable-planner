-- clone_plan, extended to carry teacher_availability rows.
--
-- Full body copied verbatim from 20260613123405_clone_plan_with_slot_bundles.sql (create
-- or replace), with one added statement that copies the source plan's availability rows,
-- remapping teacher_id through the existing _teacher_map. Modeled on the teacher/course
-- join blocks (teacher-keyed remap), NOT the coordinate-copy slot_bundles block. The
-- insert sits after the teachers block (so remapped teacher rows exist) and before the
-- map drops. Keeps SECURITY INVOKER and the same signature.
