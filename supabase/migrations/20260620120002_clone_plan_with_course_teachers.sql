-- clone_plan, extended to carry course_teachers junction rows.
--
-- Full body copied verbatim from 20260613130001_clone_plan_with_teacher_availability.sql
-- (create or replace), with one added statement that copies the source plan's
-- course_teachers rows, DOUBLE-remapping course_id via _course_map and teacher_id via
-- _teacher_map — a copy of the course_grouping_members block (the other double-remap).
-- INNER JOIN both maps so a missed remap fails loudly at insert via the composite FKs.
-- The insert sits after the courses + course_overlaps/course_merges blocks (so both
-- maps are populated) and before the map drops. Keeps SECURITY INVOKER and signature.
