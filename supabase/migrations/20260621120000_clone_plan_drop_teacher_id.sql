-- clone_plan, updated for the course_teachers junction becoming the single source of a
-- course's teachers: the courses copy no longer carries the (about-to-be-dropped) scalar
-- courses.teacher_id. Full body copied verbatim from
-- 20260620120002_clone_plan_with_course_teachers.sql (create or replace), with the only
-- change in section 3 — teacher_id and its _teacher_map left join are removed. The 4b
-- course_teachers block (double-remap) already carries the teacher set forward. Must land
-- BEFORE the column drop so no clone_plan version references a dropped column. Keeps
...
begin
  ...
  -- 3. courses (teachers now live in course_teachers, copied in 4b — no teacher_id column).
  insert into public.courses (id, plan_id, cohort, name, level, group_index, hours_per_week)
  select cm.new_id, v_new_plan_id, c.cohort, c.name, c.level, c.group_index, c.hours_per_week
  ...
  -- 7b. slot_bundles overrides: coordinate copy, no UUID remap (cell-keyed, not
  -- course-keyed). The cloned plan re-bundles everything by default except the
  -- source's explicit unbundled exceptions, which carry over verbatim.
