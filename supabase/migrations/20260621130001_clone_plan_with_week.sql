-- clone_plan, updated to carry the bi-weekly week columns through clone so cloned
-- plans preserve week data. Full body copied verbatim from
-- 20260621120000_clone_plan_drop_teacher_id.sql (create or replace), with three
-- additive changes only:
--   * section 3 (courses)          — copy week_mode
--   * section 7 (placements)       — copy week
--   * section 8 (course_groupings) — copy opposite_week
-- Keeps SECURITY INVOKER and the same signature. Must land after the columns exist
-- (20260621130000) so the create-or-replace body references real columns.
...
begin
  ...
  -- 7. placements — week carried through so a cloned plan preserves A/B assignments.
  insert into public.placements (plan_id, cohort, day, period, course_id, week)
  select v_new_plan_id, p.cohort, p.day, p.period, cm.new_id, p.week
