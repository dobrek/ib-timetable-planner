-- clone_plan, updated to carry parked (shelf) bundles through clone. Full body copied
-- from 20260624120003_clone_plan_with_bundles.sql (create or replace), with these
-- additions (everything else is verbatim):
--   * add a _shelf_bundle_map (old_id -> fresh new_id) temp table, mirroring _bundle_map
--   * insert remapped shelf_bundles (section 10) BEFORE their courses — the junction's
--     composite FK (plan_id, shelf_bundle_id) -> shelf_bundles(plan_id, id) needs the
--     parent row first
--   * section 11 (shelf_bundle_courses) — DOUBLE-remap shelf_bundle_id (_shelf_bundle_map)
--     + course_id (_course_map), INNER JOIN both so a missed remap fails loudly
--   * drop _shelf_bundle_map in the eager-drop block
-- Keeps SECURITY INVOKER and the same signature. Must land after shelf_bundles +
-- shelf_bundle_courses exist (20260626120000) so the create-or-replace body references
-- real tables.
