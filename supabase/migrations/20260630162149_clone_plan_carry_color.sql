-- clone_plan, updated to carry the optional per-course `color` through clone. Full body copied
-- verbatim from 20260626120004_clone_plan_with_shelf.sql (create or replace), with the single
-- addition of `color` to the section-3 courses INSERT column list AND its SELECT — the explicit
-- column list otherwise silently drops the new column on plan-clone. Keeps SECURITY INVOKER and
-- the same signature.
