-- clone_plan, updated to carry the per-member `is_optional` flag through clone. Full body
-- copied verbatim from 20260630162149_clone_plan_carry_color.sql (create or replace), with
-- the single addition of `is_optional` to the section-7 placements INSERT column list AND
-- its SELECT, and the section-11 shelf_bundle_courses INSERT column list AND its SELECT —
-- the explicit column lists otherwise silently drop the new column on plan-clone. Keeps
-- SECURITY INVOKER and the same signature.
...
begin
  ...
  insert into public.courses (id, plan_id, cohort, name, level, group_index, hours_per_week, week_mode, color)
  select cm.new_id, v_new_plan_id, c.cohort, c.name, c.level, c.group_index, c.hours_per_week, c.week_mode, c.color
