-- clone_plan, updated to carry the per-course `finishes_early` flag through clone. Full
-- body copied verbatim from the latest live definition
-- (20260707120004_clone_plan_carry_optional.sql, create or replace), with the single
-- addition of `finishes_early` to the section-3 courses INSERT column list AND its SELECT
-- — the explicit column list otherwise silently drops the new column on plan-clone,
-- resetting every course's flag to the `false` default. Keeps SECURITY INVOKER and the
-- same signature.
