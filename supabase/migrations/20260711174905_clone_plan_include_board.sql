-- clone_plan, extended with an optional `p_include_board` parameter so an author can
-- clone the catalog alone (teachers + availability, courses, course_teachers, overlaps,
-- merges, students, choices) while the board (bundles, placements, groupings + members,
-- shelf bundles + members) resets to empty. Full body copied verbatim from the latest
-- live definition (20260711133933_clone_plan_carry_finishes_early.sql) with the single
-- structural change of wrapping the six board-copy blocks (6b–11) in
-- `if p_include_board then … end if;`. Catalog blocks, the plans insert, temp-map
-- creation, and the eager temp-table drops stay unconditional. Keeps SECURITY INVOKER
-- and `set search_path = ''`.
--
-- The 2-arg function MUST be dropped first: a 3-arg `create or replace` with a default
-- would create an *overload* rather than replace it, making a named-param RPC call
-- ("function is not unique") ambiguous. The default `p_include_board => true` keeps every
-- existing 2-arg-style caller on the current full-clone behavior (additive, non-breaking).
...
begin
  ...
  -- search_path is empty. The three board maps (_grouping_map, _bundle_map,
  -- _shelf_bundle_map) go unused when p_include_board is false — harmless.
  ...
  create temp table _shelf_bundle_map (
    old_id uuid primary key,
    new_id uuid not null default gen_random_uuid()
  ) on commit drop;
  ...
  insert into pg_temp._shelf_bundle_map (old_id)
  select id from public.shelf_bundles where plan_id = p_source_plan_id;
  ...
  -- color carried through so a cloned plan preserves the optional per-course subject color.
  -- finishes_early carried through so a cloned plan preserves the early-finish flag.
  insert into public.courses (id, plan_id, cohort, name, level, group_index, hours_per_week, week_mode, color, finishes_early)
  select cm.new_id, v_new_plan_id, c.cohort, c.name, c.level, c.group_index, c.hours_per_week, c.week_mode, c.color, c.finishes_early
  ...
  -- Board copy (sections 6b–11). Gated on p_include_board: when false, the clone lands
  -- in the exact cold-start state of a freshly created blank plan — an empty-but-
  -- interactive board whose only next step is "Compute groupings" per cohort. FK
  -- direction is one-way (board → catalog, never catalog → board), so skipping these
  -- blocks leaves zero dangling FK and no NOT-NULL violation. Any FUTURE board-table
  -- copy MUST land inside this guard.
  if p_include_board then
    ...
    -- is_optional carried through so a cloned plan preserves pending optional decisions.
    insert into public.placements (plan_id, cohort, day, period, course_id, week, bundle_id, is_optional)
    select v_new_plan_id, p.cohort, p.day, p.period, cm.new_id, p.week, bm.new_id, p.is_optional
    ...
    -- 10. shelf_bundles — remapped to fresh UUIDs (cohort copied as-is). MUST precede
    -- section 11: each cloned shelf_bundle_courses row's composite FK
    -- (plan_id, shelf_bundle_id) -> shelf_bundles(plan_id, id) requires the parent first.
    insert into public.shelf_bundles (id, plan_id, cohort)
    select sm.new_id, v_new_plan_id, s.cohort
      from public.shelf_bundles s
      join pg_temp._shelf_bundle_map sm on sm.old_id = s.id;

    -- 11. shelf_bundle_courses — DOUBLE-remap shelf_bundle_id (_shelf_bundle_map) +
    -- course_id (_course_map). INNER JOIN both so a stale/foreign UUID fails loudly via the
    -- composite FKs — otherwise a duplicated plan would silently drop its parked course set.
    -- week carried through so a parked A/B course preserves its formation on clone.
    -- is_optional carried through so a parked optional member preserves its flag on clone.
    insert into public.shelf_bundle_courses (plan_id, shelf_bundle_id, course_id, week, is_optional)
    select v_new_plan_id, sm.new_id, cm.new_id, sc.week, sc.is_optional
  ...
  drop table pg_temp._shelf_bundle_map;
